...
- Commands live in `.agents/commands/<category>/<name>.md`.
...
- If your Cursor version only discovers top-level `.md` files, use a flat layout or symlink each
  command into `.agents/commands/` (e.g. `feature.md` → `workflow/feature.md` symlinked as
  `feature.md` in the root of commands).

## Adding a New Command

1. Choose the category (bootstrap, git, workflow, quality, docs, devops, ideation).
2. Create `.agents/commands/<category>/<command-name>.md` following existing command structure.
