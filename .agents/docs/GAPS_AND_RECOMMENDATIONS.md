...
files. Skills live in `.agents/skills/<skill-name>/SKILL.md` (project) or `~/.agents/skills/`
(user). The agent can discover and invoke them for domain-specific tasks.

**Current state:** All five recommended skills have been created in `.agents/skills/`. See
...
| -------------------------------------- | ------------------------------------------------------------------------------------ |
| `.agents/skills/create-rule/`          | How to write and structure a Cursor rule (`.mdc`).                                   |
| `.agents/skills/create-command/`       | Deep expertise for creating a new command: structure, steps, integration with rules. |
| `.agents/skills/security-review/`      | Checklist and reasoning for security review before merge.                            |
| `.agents/skills/conventional-commits/` | When to use feat/fix/docs/refactor/chore, how to write a good subject and body.      |
| `.agents/skills/release-notes/`        | How to derive release notes from commits, what to include in a GitHub release body.  |
...
- **README or INSTALLATION:** ~~Add one line that the playbook can optionally include Cursor Agent
  Skills in `.agents/skills/` and link to this doc.~~ Done -- README updated.
- **COMMANDS.md / USER_GUIDE.md:** ~~When skills exist, add a short "Skills" section listing
  `.agents/skills/*` and when the agent might use them.~~ Done -- COMMANDS.md updated with skills
  table.
...
| Gap              | Status  | Notes                                                                                                |
| ---------------- | ------- | ---------------------------------------------------------------------------------------------------- |
| **Agent skills** | Done    | 5 skills created in `.agents/skills/`. CONCEPTS.md explains skills, rules, commands, and MCP.        |
