#!/bin/bash
# pre-tool-use.sh — Triggered before every Bash tool use
...
if [ -n "$SETUP" ]; then
    ...
    if echo "$INPUT" | grep -qE "(sed -i|awk.*>|patch )"; then
        ...
        if [ -z "$CHAR_TESTS" ]; then
            log "WARN: legacy modification with no characterization tests detected"
            # Warn only — don't block; the guardian handles it
        fi
    fi
fi
