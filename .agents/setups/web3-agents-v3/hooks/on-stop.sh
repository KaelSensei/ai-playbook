#!/bin/bash
# on-stop.sh — Triggered by the Claude Code "Stop" hook
# Runs when Claude finishes a complete response
# Role: check whether the current task is done and kick off the next one
...
# ── If task is IDLE → check the queue ────────────────────────────
...
if [ "$STATUS" = "IDLE" ]; then
    ...
    if [ -z "$NEXT" ]; then
        log "Queue empty — supervisor idling"
        exit 0
    fi

    log "Next task detected: $NEXT"

    # Write the next task into a "pending" file
...
if task_name and task_name != "none":
    ...
    content = re.sub(
        ...
        f'- [x] {task_name} _(completed on {timestamp})_\n',
        content
    )
