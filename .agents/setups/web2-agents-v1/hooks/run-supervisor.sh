...
# Uses native Claude Code hooks (.claude/hooks/)
# Launched in background by startup.sh
...
Instructions:
1. Analyze complexity (level 1/2/3)
2. Select the MINIMUM agents required
3. Run the FULL pipeline to completion without stopping
4. Include /pr automatically if any code was produced
...
while true; do
    ...
    # Check whether the on-stop hook has queued a next task
    TASK=$(get_next_task)
