#!/bin/bash
# post-tool-use.sh — Triggered after every Bash or Write tool call
# Role: detect progress markers, update state
...
if echo "$OUTPUT" | grep -qE "(PASS|✓|passing|Tests:.*passed)"; then
    log "Green tests detected — GREEN phase validated"
...
if echo "$OUTPUT" | grep -qE "(FAIL|✗|failing|AssertionError)"; then
    log "Red tests detected — RED phase validated"
fi
