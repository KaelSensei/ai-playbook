...
during tests to correlate what you see in tests with what's happening in the systems. A test that
passes but whose p99 is at 2s is a failure.
...
# Integrated in Playwright tests:
...
```bash
# Contract tests: verify that endpoints honor their contract
...
Live observability metrics read during the tests
Results compared against TEST_PLAN.md thresholds
```
...
# → read from the Sentry API or dashboard
...
PASS ✅ / FAIL ❌ / WARN ⚠️ (passes but monitor)
...
- Always compare against TEST_PLAN.md thresholds — no subjective judgment
- A perf regression = FAIL even if functional tests pass
- Always read observability during load tests
- Mandatory smoke tests before any merge
