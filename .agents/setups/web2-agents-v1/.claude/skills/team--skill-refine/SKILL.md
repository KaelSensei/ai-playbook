...
## On REQUEST_REDESIGN (immediate stop)
...
5. If overridden → document in `tasks/current_task.md`: _"[date] [agent] → risk accepted: [issue] —
...
- Agents without improvements → their previous verdict still applies
...
- If after loop 3 an agent still returns `APPROVE_WITH_CHANGES` on the same issue → escalate: _"3
  iterations on [issue]. User decision required."_
...
Never resolve conflicts by picking the "nicest" option or the one from the most senior agent.
...
- Each agent's previous verdict (so they can check that their issues were addressed)
- The diff of what has changed since their last review
- Unchanged context docs

The orchestrator tracks:

- Issues raised by agent by loop
